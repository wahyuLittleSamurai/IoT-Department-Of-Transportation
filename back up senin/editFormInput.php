<?php
	...
	else
	{
	    if(!empty($_GET["idKendaraanUji"]))
    	{
    	    ...
    	    $queryCommand = mysqli_query($link, "SELECT * FROM tblkendaraan WHERE idKendaraan = '$grabIdKendaraan'");
		...
		if(isset($_POST["btnInsert"]) && !empty($_POST["txtBeratKendaraan"]) && !empty($_POST["txtJenisKendaraan"]) && !empty($_POST["txtNomorKendaraan"]) && !empty($_POST["txtBeratKosong"]))
		{
			...
			$kelasKendaraan = $_POST["valKelas"];

			
			$sqlFile = "UPDATE tblkendaraan SET kelas='$kelasKendaraan',jenis='$jenisKendaraan',noPol='$nomorKendaraan',
			            kosong='$beratKosong',maximal='$beratKendaraan',time='$getDate' WHERE idKendaraan='$grabIdKendaraan'";
			
			$queryFile = mysqli_query($link, $sqlFile);
			
			$sqlFileReport = "UPDATE tbluji SET kelas='$kelasKendaraan',jenis='$jenisKendaraan',noPol='$nomorKendaraan',
			            kosong='$beratKosong',maximal='$beratKendaraan' WHERE idKendaraan='$grabIdKendaraan'";
			
			$queryFileReport = mysqli_query($link, $sqlFileReport);
		...
		<div class="container">
			...
			<form method="POST" role="form" style="width:400px; margin: 0 auto;">
				...
				<div class="required-field-block">
					<input type="text" name="txtJenisKendaraan" value = "<?php echo $grabJenis; ?>" placeholder="Jenis Kendaraan" class="form-control">
				...
				<div class="required-field-block">
					<input type="text" name="txtNomorKendaraan" value = "<?php echo $grabNoPol; ?>" placeholder="Nomor Kendaraan" class="form-control">
					<div class="required-icon">
						<div class="text">*</div>
					</div>
				</div>
				<div class="required-field-block">
					<input type="text" name="txtBeratKosong" value = "<?php echo $grabKosong; ?>" placeholder="Berat Kosong" class="form-control">
					<div class="required-icon">
						<div class="text">*</div>
					</div>
				</div>
				...
				<div class="required-field-block">
					<input type="text" name="txtBeratKendaraan" value = "<?php echo $grabMaximal; ?>" placeholder="Berat Maksimal" class="form-control">
					<div class="required-icon">
						<div class="text">*</div>
					</div>
				</div>
